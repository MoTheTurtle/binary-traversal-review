public class TraversalReview {
    ...
    public static int tripleSum(TreeNode node) {
        ...
        int node1 = 3*node.data;
        int left = tripleSum(node.left);
        int right = tripleSum(node.right);
        return node1+left+right;
    }
    ...
    public static int evenMax(TreeNode node) {
        ...
        int leftM = evenMax(node.left);
        int rightM = evenMax(node.right);
        if(leftM > max && leftM %2 == 0){
            max = leftM;
        }
        if(rightM > max && rightM %2 == 0){
            max = rightM;
        }
        return max; 
    }
    ...
    public static boolean noSingleChildren(TreeNode node) {
        ...
        return children && noSingleChildren(node.left) && noSingleChildren(node.right);
    }
